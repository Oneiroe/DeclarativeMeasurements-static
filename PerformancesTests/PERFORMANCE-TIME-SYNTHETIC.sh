...
#for MAX_STRLEN in {6200..1000000..100}; do
for TESTBED_SIZE in {3500..1000000..50}; do
    echo "########### Log size:"${TESTBED_SIZE}" ----------------iteration:"${ITERATION}
    ###################################################################
    ## Generate log
    #MIN_STRLEN=$((MAX_STRLEN-100))
    echo "########### Generate Log"
    ORIGINAL_GENERATED_LOG=${TEST_FOLDER}/${TEST_BASE_NAME}/${TEST_BASE_NAME}"-log[min_${MIN_STRLEN}_max_${MAX_STRLEN}_size_${TESTBED_SIZE}].txt"
...
#        echo ${LOG}
    ...
    -iLE ${LOG_ENCODING} \
    -iMF ${TEST_FOLDER}/${TEST_BASE_NAME}-checking-model.json \
    -iME ${MODEL_ENCODING}
